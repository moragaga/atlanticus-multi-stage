...
class WebRuntimeInfrastructure:
    def __init__(
        ...
        *,
        ...
        if any(
            not isinstance(settings, CosmosSettings) for settings in copied_connections.values()
        ):
    ...
    def close(self) -> None:
        ...
        self._closed = True
        if errors:
            raise RuntimeInfrastructureError('Could not close runtime infrastructure') from errors[
                0
            ]
