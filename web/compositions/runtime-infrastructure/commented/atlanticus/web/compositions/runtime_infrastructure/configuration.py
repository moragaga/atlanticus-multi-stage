...
# Declara una conexión lógica sin imponer nombres de environment ni nombres de base a las
# soluciones.
# Una aplicación puede construir tantas definiciones como necesite y elegir libremente las
# variables.
...
# SharePoint sigue el mismo principio: el contrato conoce roles de variables, no nombres
# universales.
@dataclass(frozen=True, slots=True)
...
# Resultado ya resuelto que consume WebRuntimeInfrastructure; no conserva acceso al environment.
@dataclass(frozen=True, slots=True)
...
# Convierte definiciones declarativas y valores ya inyectados en settings propios de Connectivity.
...
# Resuelve ambos endpoints Power Automate y sus rutas sin distinguir local, Azure u otro ambiente.
...
# Modelo interno del endpoint firmado una vez separado en transporte, path y parámetros sensibles.
@dataclass(frozen=True, slots=True)
...
# Los nombres lógicos se validan sin aplicar ninguna convención de prefijos o dominio.
...
# Los nombres de variables son datos declarativos; esta validación solo evita nombres imposibles.
