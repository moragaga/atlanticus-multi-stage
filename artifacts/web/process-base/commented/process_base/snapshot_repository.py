# Espejo comentado: fuente fake de laboratorio; el contrato es el mismo que usará un repository compartido real.
...
# Fake repository compartido para certificar los tres canales sin conectar Cosmos todavía.
class ProcessBaseSnapshotRepository(SnapshotRepository):
    def __init__(self, definition: DashboardDefinition) -> None:
        self._definition = definition

    # Cada canal publica una revisión global estable durante su bucket de cadencia.
    ...
    # Las series respetan [start_utc, end_utc), step_seconds y no transportan timestamps por punto.
    ...
    def _require_tool(self, tool_key: str) -> None:
        if tool_key != self._definition.manifest.tool_key:
            raise KeyError(f'Unknown process base tool: {tool_key!r}')


def _value(revision: str, component_index: int, child_index: int) -> float:
    seed = int(revision[-8:]) + component_index * 23 + child_index * 11
    return round(50 + (seed % 700) / 10, 1)
...
def _series_values(
    ...
    *,
    ...
    seed = int(revision[-8:]) + component_index * 31 + series_index * 17
    base = 70 + (seed % 250) / 10
