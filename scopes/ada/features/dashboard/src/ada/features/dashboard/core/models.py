...
from .projections import ComponentDataSnapshot, ComponentTimeSeriesSnapshot
...
class ComponentBundle:
    ...
    def __post_init__(self) -> None:
        ...
        if any(
            not isinstance(hours, int) or not isinstance(window, TimeSeriesWindow)
...
def build_component_bundle(
    *,
    ...
    if data_snapshot is not None and data_snapshot.component_key != component_key:
        raise DashboardDefinitionError(
            'Data snapshot component key does not match bundle component'
        )
