    ...
    for key, state_value in raw_states.items():
        if not isinstance(state_value, str):
            raise DashboardStoreError(
                f'Component projection state for {key!r} must be a string'
            )
        try:
...
def _json_value(value: object) -> JsonValue:
    ...
    raise DashboardStoreError(
        f'Dashboard store value is not JSON-compatible: {type(value).__name__}'
    )
...
def _datetime(mapping: Mapping[str, object], key: str) -> datetime:
    ...
    try:
        ...
        raise DashboardStoreError(
            f'Dashboard store field {key!r} must be an ISO datetime'
        ) from error
