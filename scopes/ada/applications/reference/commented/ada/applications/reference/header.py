# Este escenario de referencia usa el Header real para comparar la geometría estándar.
# Las combinaciones 2/3 measurements y last opcional son deliberadas para QA visual.
...
class _ReferenceIndicatorDefinition:
    # Identidad y contenido visible del indicador de QA.
    key: str
    label: str
    unit: str
    scope: ToolScope
    plan: str
    # El orden declarado es el mismo orden que debe renderizar el Header.
    measurements: tuple[tuple[str, str], ...]
    include_last_measurement: bool = False


# Esta matriz cubre las geometrías normales que queremos inspeccionar en navegador.
...
def build_reference_header_state(services: ServiceRegistry, manifest: ToolManifest):
    ...
    return build_reference_header_state_from_values(
        manifest,
        values={definition.key: snapshot.value(definition.key) for definition in _INDICATOR_DEFINITIONS},
...
def _indicator(
    ...
    *,
    ...
    # El scope decide el destino del Header; no se infiere desde el label.
    ...
    return HeaderIndicatorPlacement(
        ...
        indicator=GlobalIndicatorState(
            ...
            # El slot existe visualmente aunque este estado sea None.
            last_measurement=(
