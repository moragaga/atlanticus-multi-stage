...
class ReferenceDashboardCatalog:
    ...
    def definitions(self) -> Mapping[str, DashboardDefinition]:
        return {tool_key: dashboard.definition for tool_key, dashboard in self.dashboards.items()}
...
def _card_content(
    ...
    *,
    ...
    series_window = next(
        (window for window in bundle.time_series.values() if subcomponent in window.series),
        None,
    )
    children = [
        html.Div(
