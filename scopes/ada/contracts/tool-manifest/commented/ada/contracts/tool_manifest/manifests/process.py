...
# Process recibe regiones funcionales reales; el rol geométrico viaja en cada REGION.
def build_process_manifest(
    *,
    ...
    # Primero validamos la declaración independiente antes de componer las secciones comunes.
    _validate_process_section_declarations(
        sections=resolved_sections,
        operational_scope=operational_scope,
    )

    # Global indicators y time status son unidades configurables completas para KPI y alarmas.
...
# Cada región directa del body debe tener un rol único; CENTER es obligatorio y define la zona divisible.
def _validate_process_section_declarations(
    *,
    ...
    for region in regions:
        expected_targets = (
            _KPI_ALARM if region.layout_role is ProcessBodySection.CENTER else _KPI
        )
...
# Solo los descendientes de la región CENTER pueden existir como targets internos y son exclusivamente alarmables.
