...
def _build_source(source: TimeStatusSourceState, *, now: datetime) -> html.Div:
    ...
    if updated_at is not None:
        attributes['data-updated-at-utc'] = updated_at.isoformat().replace('+00:00', 'Z')

    icon_class, value = _resolve_source_content(source, visual_state=visual_state, now=now)
    return html.Div(
...
def _resolve_source_content(
    ...
    *,
    ...
    icon = 'bi bi-cloud-slash' if visual_state == Freshness.STALE.value else 'bi bi-cloud-check'
