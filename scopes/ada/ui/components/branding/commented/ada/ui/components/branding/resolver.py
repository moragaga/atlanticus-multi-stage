"""Resolución determinista de la variante de marca que debe usar la UI."""
...
def resolve_brand(manifest: BrandManifest, context: BrandContext) -> BrandState:
    # Un override a default desactiva cualquier celebración del calendario.
    if context.requested_variant == 'default':
        return _default_state(manifest)

    # Una variante explícita tiene precedencia y puede probarse fuera de su ventana.
    ...
    if not active:
        return _default_state(manifest)
    if len(active) > 1:
        # La ambigüedad se rechaza para no esconder una mala configuración de calendario.
...
def _variant_state(manifest: BrandManifest, variant: BrandVariant) -> BrandState:
    # Mientras no exista un asset festivo aprobado, se conserva el logo base de forma segura.
