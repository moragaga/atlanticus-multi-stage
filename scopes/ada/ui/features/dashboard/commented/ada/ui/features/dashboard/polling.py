# Polling agregado por herramienta/canal: compara una revisión global y distribuye el snapshot sólo cuando cambia.
...
# Resultado de una lectura nueva: una única revisión global y los valores proyectados para los Stores.
class DashboardChannelUpdate:
    revision: str
    component_values: Mapping[str, object]


# Los canales activos se deducen de los componentes realmente implementados; STATUS acompaña a toda unidad activa.
...
# Registra un callback por canal usando un único Interval compartido por la herramienta.
...
# El SharedSnapshotReader resuelve microcaché/TTL; aquí sólo se transforma el snapshot agregado para la UI.
...
# Ante fallo se conserva el último estado del navegador devolviendo no_update para todos los outputs del canal.
def _poll_callback(
    _n_intervals: object,
