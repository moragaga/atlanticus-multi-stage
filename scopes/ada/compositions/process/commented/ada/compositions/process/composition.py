...
# La composición conserva Dashboard como contenido y agrega opcionalmente la autoridad runtime de Tool Projection.
...
class ProcessToolComposition:
    ...
    def build_body(self, *, layout_id: str | None = None) -> html.Div:
        # Las cards reciben identidad de subcomponente y el layout recibe identidad de componente.
        content = {
    ...
    def build_tool(
        ...
        *,
        ...
        # El mismo mapa de wrapper ids alimenta Header, Time Status y Body.
...
def create_process_tool_composition(
    ...
    *,
    ...
    # La proyección es opcional hasta que el artifact la entregue, pero si existe debe coincidir exactamente.
    ...
    # Los stores R3 se materializan sólo cuando existe una Tool Projection real.
...
def _build_component_cards(
    ...
    *,
    ...
    for section in manifest.children(component_key):
        ...
        # Cada card conserva un único wrapper canónico derivado de la proyección.
        cards.append(
...
def _build_runtime_store_mount(
    ...
    if projection is None:
        return None
    # El registry existente traduce los bindings de Tool Projection a Latest/Timeseries stores.
...
def _runtime_hosts(
    ...
    # Dashboard host y R3 store host conviven ocultos sin alterar la geometría visible.
...
def _component_wrapper_ids(
    ...
    if projection is None:
        return {}
    # El mapping se usa por clave de componente, nunca por posición visual.
...
def _subcomponent_wrapper_id(
    ...
    *,
    ...
    if projection is None:
        return None
    # ToolRuntimeBindings resuelve además los subcomponentes compartidos por sus linked components.
...
def _validate_projection(
    ...
    # Primero se entrega un error de identidad claro y luego se exige igualdad estructural completa.
