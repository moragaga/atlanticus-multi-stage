...
# Describe los bindings que una superficie puede consumir para un componente Tool estable.
...
class RuntimeComponentStoreSpec:
    ...
    def __post_init__(self) -> None:
        # Normaliza y valida identidad y bindings sin inventar nombres fuera de Tool Projection.
        values = (
...
# Registry inmutable que centraliza el naming runtime y evita strings repetidos en callbacks UI.
@dataclass(frozen=True, slots=True)
class RuntimeComponentStoreRegistry:
    ...
    # Resuelve el contrato completo de un componente desde su key estable.
    ...
    # Entrega el ID Latest sin que la UI reconstruya manualmente el naming.
    def latest(self, component_key: str) -> str:
        return self.component(component_key).latest_store_id

    # Entrega el ID Timeseries con la misma autoridad contractual.
    def timeseries(self, component_key: str) -> str:
        return self.component(component_key).timeseries_store_id


# Compila el registry exclusivamente desde una Tool Projection ya validada por Tools.
