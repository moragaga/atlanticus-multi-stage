...
ARTIFACT_DIR="$ROOT_DIR/artifacts/web/integrated-operations"
SMOKE_DIR="$ROOT_DIR/integration/integrated-operations-runtime"
IMAGE="atlanticus-integrated-operations:local"
SUFFIX="${$}"
NETWORK="atlanticus-r18c-${SUFFIX}"
...
umask 077
{
    ...
    printf 'ATLANTICUS_SMOKE_APPLICATION_URL=http://integrated-operations:8000\n'
} > "$ENV_FILE"
...
docker run -d \
    --name "$APP_CONTAINER" \
    --network "$NETWORK" \
    --hostname integrated-operations \
    --env-file "$ENV_FILE" \
    --memory 768m \
    --cpus 0.75 \
    "$IMAGE" >/dev/null
...
if ! docker ps --filter "name=^/${APP_CONTAINER}$" --filter status=running --format '{{.Names}}' | grep -Fxq "$APP_CONTAINER"; then
    echo 'Integrated Operations container did not remain running.' >&2
