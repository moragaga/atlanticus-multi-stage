...
def main() -> None:
    ...
    assert 'application' in result.databases_created
    assert set(result.containers_created['application']) == {
        'users',
        'users_support',
        'user_activity',
        'configuration',
    }
    print(f"Access smoke Cosmos provisioned: {os.environ['ATLANTICUS_COSMOS_DATABASE']}")
